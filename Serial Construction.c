...
void createSuffixArray(suffix_struct* ss)
{
	...
	for (i = 1; i < FTAB_SIZE + 1; i++)	//Cumulative frequencies to find the occurrences easily
	{
		ftab[i] += ftab[i-1];
	}
	...
	for(i=1;i<FTAB_SIZE+1;i++)		//Send buckets for sorting
	{
		if (ftab[i]-ftab[i-1]>0)	//If there is an occurrence of that character
			sub_sort(ss,ftab[i-1],ftab[i]-1,1);
	}
}
